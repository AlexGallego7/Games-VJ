...
void greyBook::init(const glm::ivec2& tileMapPos, ShaderProgram& shaderProgram)
{
...
void greyBook::update(int deltaTime)
{
...
void greyBook::render()
{
	if (!cat)sprite->render();
}

void greyBook::setTileMap(TileMap* tileMap)
{
	map = tileMap;
}

void greyBook::setPosition(const glm::vec2& pos)
{
	posPlayer = pos;
	sprite->setPosition(glm::vec2(float(tileMapDispl.x + posPlayer.x), float(tileMapDispl.y + posPlayer.y)));
}

int greyBook::getTypeEntity() {
	return 11;
}
...
bool greyBook::LeftMove() {
	return 0;
}

bool greyBook::IsClimbing() {
	return 0;
}


int greyBook::getState() {
